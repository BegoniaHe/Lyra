...
NSStringEncoding encodingFromIANA(NSString *ianaName) {
    ...
    CFStringEncoding cfEncoding = CFStringConvertIANACharSetNameToEncoding((__bridge CFStringRef)ianaName);
...
NSArray<NSString *> *fallbackEncodingsForProfile(EncodingProfile profile) {
    switch (profile) {
        case EncodingProfile::ID3v1:
            return @[@"UTF-8", @"windows-1252", @"ISO-8859-1", @"Shift_JIS", @"windows-1251", @"Big5"];
...
NSString *smartDecodeData(NSData *source, EncodingProfile profile) {
    ...
    TagLib::ByteVector buffer(static_cast<const char *>(source.bytes), static_cast<unsigned int>(source.length));
...
BOOL isLikelyMojibake(NSString *value) {
    ...
    for (NSUInteger i = 0; i < value.length; i++) {
        ...
        if ([[NSCharacterSet controlCharacterSet] characterIsMember:c] && c != '\n' && c != '\r' && c != '\t') {
            continue;
        }
        printableCount++;
    ...
    const double latinRatio = static_cast<double>(latinSupplementCount) / static_cast<double>(printableCount);
    return latinRatio >= 0.4 && cjkCount == 0;
}

void fillStringWithRepair(NSMutableDictionary *metadata,
                          NSString *field,
...
void applyAIFFChunkFallback(NSString *path, NSMutableDictionary *metadata) {
    NSString *ext = [[path pathExtension] lowercaseString];
    if (!([ext isEqualToString:@"aif"] || [ext isEqualToString:@"aiff"] || [ext isEqualToString:@"aifc"])) {
        return;
    }
    ...
    while (offset + 8 <= fileData.length) {
        ...
        NSData *chunkData = [NSData dataWithBytes:(bytes + offset) length:chunkSize];
...
class SmartRIFFInfoStringHandler final : public TagLib::RIFF::Info::StringHandler {
public:
    TagLib::String parse(const TagLib::ByteVector &data) const override {
...
class SmartID3v2Latin1Handler final : public TagLib::ID3v2::Latin1StringHandler {
public:
    TagLib::String parse(const TagLib::ByteVector &data) const override {
        if (NSString *decoded = smartDecode(data, EncodingProfile::ID3v2Latin1Fallback)) {
            return taglibStringFromNSString(decoded);
        }
...
void applyAVFoundationFallback(NSString *path, NSMutableDictionary *metadata) {
...
void applyAVAudioPropertiesFallback(NSString *path, NSMutableDictionary *metadata) {
    NSURL *url = [NSURL fileURLWithPath:path];
    AVURLAsset *asset = [AVURLAsset URLAssetWithURL:url options:nil];
...
+ (NSDictionary<NSString *, id> *)readMetadataFromFile:(NSString *)path {
    ...
    applyAIFFChunkFallback(path, metadata);

    applyAVFoundationFallback(path, metadata);

    applyAVAudioPropertiesFallback(path, metadata);
